...
# +
    ...
    for temp in range(total_trials): 
        ...
        Network_to_test = Net_vanilla_NN_small(input_channels,hidden_ratio[-1],num_train_labels)
        
        for i in range(len(hidden_ratio)):
            Networks_to_train.append(Net_vanilla_NN_small(input_channels,hidden_ratio[i],num_train_labels))
            Networks_to_train_noise.append(Net_vanilla_NN_small(input_channels,hidden_ratio[i],num_train_labels))
...
#             print(i)
        ...
        Network_to_test,normal_loss = train_network_normal(Network_to_test,trainloader, init_rate,total_epochs, 10,0.8, decay_normal)
#         print(my_dataset.labels)
#         print(my_dataset_test.labsels)
        accuracy = test_network(Network_to_test, testloader, my_dataset_test.labels)
        train_accuracy = test_network(Network_to_test, trainloader, my_dataset.labels)
    ...
    with open('Adam_datasizevariant'+str(variable_datasize)+'_correlation_test_performance_'+args.dataset +'_' + str(num_train_labels)+'classes_'+ str(total_trials)+'trials_FCNN.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
