...
class Net_vanilla_NN_small(nn.Module):
    def __init__(self,input_channels,hidden_ratio=1.0,classes = 10):
        ...
        pads = (np.array(kernel_sizes) - 1) / 2
        pads = pads.astype(int)
        layers = [int(60*hidden_ratio), int(30*hidden_ratio),int(60*hidden_ratio)]
        self.post_filter = False
        # network layers
    ...
    def forward(self, x):
        ...
        # xm = x_checkpoint.view(
            # [x_checkpoint.shape[0], x_checkpoint.shape[1] * x_checkpoint.shape[2] * x_checkpoint.shape[3], 1, 1])
        xm = self.fc_direct(x_checkpoint)
        # xm = self.fc1(xm)
        # xm = self.relu(self.fc1bn(xm))
        # # xm = self.dropout(xm)
        # xm = self.fc2(xm)
...
#     px_list = ['c1', 'c2', 'f2', 
#                'f3',   'l2', 'l3',  'n4', 't2', 't3', 't4']
# #     px_list = ['c1', 'c2', 'density', 'f2', 
#                'f3', 'f4', 'l1', 'l2', 'l3', 'n2', 'n4', 't2', 't3', 't4']
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
...
#         Network_to_test = Net_vanilla_cnn_small(input_channels,2.0,num_train_labels)
        Network_to_test = Net_vanilla_NN_small(input_channels,2.0,num_train_labels)
...
#         print(int(torch.numel(dataset2.data)/dataset2.data.shape[0]))
        dataset2.data = dataset2.data.reshape((dataset2.data.shape[0],int(torch.numel(dataset2.data)/dataset2.data.shape[0])))
        print(dataset2.data.shape)
        print(dataset2.targets.shape)
        G = px.classification
        measures = []
        for func in getmembers(G, isfunction):
            time_start = time.time()
            if func[0] in px_list:
                time_start = time.time()
                measures.append(func[1](dataset2.data.cpu().numpy(),dataset2.targets.cpu().numpy()))
                print(func[0],":", time.time()-time_start)
        
        all_measures.append(measures)

    with open('Problexity_datasizevariant'+str(variable_datasize)+'_Correlation_test_performance_'+args.dataset +'_' + str(num_train_labels)+'classes_'+ str(total_trials)+'trials_NN.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([px_list,all_measures,all_accuracies,all_train_accuracies], f)
